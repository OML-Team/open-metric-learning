...
def generate_retrieval_case(
    ...
    """
    Function creates random `labels`, `is_query`, `is_gallery` set of parameters
    and `distance_matrix` or `desired_positions` of GT
    ...
    for label, num_samples_for_label in enumerate(num_samples):
        ...
        # we guarantee that there are at least one query and one gallery for each label
    ...
    for is_query_for_label, is_gallery_for_label in zip(is_query, is_gallery):
        for q, g in zip(is_query_for_label, is_gallery_for_label):
            if q:
                desired_correct_positions.append(_sample_elements_from_range(num_galleries_total - 1 * (q == g),
                                                                             sum(is_gallery_for_label) - 1 * (q == g)))
    ...
    if return_desired_correct_positions:
        ...
        distances = generate_distance_matrix(desired_correct_positions,
                                             labels=labels,
                                             is_query=is_query,
...
def generate_distance_matrix(correct_positions_array: List[List[int]],
                             labels: torch.Tensor,
                             is_query: torch.Tensor,
                             is_gallery: torch.Tensor) -> torch.Tensor:
    """
    Function generates distance matrix based on desired correct positions.
    For cases query == zero coresponded distances are equal to zero
    ...
    q_is_g_array = torch.any(mask_to_ignore, dim=1).tolist()
    query_ids = torch.nonzero(is_query).squeeze()

    assert all(len(pos) < num_gallery for pos in correct_positions_array)

    for idx, (pos, q_is_g) in enumerate(zip(correct_positions_array, q_is_g_array)):
        if not all(idx < num_gallery - int(q_is_g) for idx in pos):
            raise ValueError(f"Desired ids are greater than available number of galleries for "
                             f"label_idx={query_ids[idx].item()}, desired_positions={pos}")
        if not (len(pos) < (num_gallery - int(q_is_g))):
            raise ValueError(f"Desired number of ids are greater than available number of galleries for "
                             f"label_idx={query_ids[idx].item()}, desired_positions={pos}")
    ...
    for row, (query_idx, correct_positions, q_is_g) in enumerate(zip(query_ids, correct_positions_array, q_is_g_array)):
        ...
        rest_of_distances = set(range(num_gallery - int(q_is_g))) - set(correct_positions)
