...
def assert_signature(fn: Callable):
    ...
    if len(parameters) < 2 or parameters[0] != 'rank' or parameters[1] != 'world_size':
        raise ValueError(
            f"The function '{fn.__name__}' should have 'rank' and 'world_size' as the first two arguments")


def generate_connection_filename(world_size: int, fn: Callable, *args: Tuple[Any, ...]) -> Path:
...
def fn_ddp_wrapper(rank: int, connection_file: Path, world_size: int, fn: Callable, *args: Tuple[Any, ...]) -> Any:  # type: ignore
    init_process_group(
        backend="gloo",
        rank=rank,
