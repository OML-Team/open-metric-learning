...
from oml.functional.metrics import calc_mask_to_ignore, calc_gt_mask, apply_mask_to_ignore
...
def test_syntetic_cases(max_num_labels: int, max_num_samples_per_label: int, num_attempts: int) -> None:
...
def check_distance_matrix(case: TCase) -> None:
    ...
    distances = generate_distance_matrix(desired_positions_array,
                                         labels=labels,
                                         is_query=is_query,
    ...
    for query_label, ids_sorted_for_query, desired_positions in \
            zip(labels[is_query], ids_sorted_distance, desired_positions_array):
