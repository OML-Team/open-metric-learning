...
from oml.functional.metrics import calc_retrieval_metrics, calc_mask_to_ignore, calc_gt_mask, TMetricsDict
...
def _naive_cmc(positions: TPositions, k: int) -> torch.Tensor:
...
def _naive_map(positions: TPositions, k: int) -> torch.Tensor:
...
def _naive_precision(positions: TPositions, k: int) -> torch.Tensor:
    values = torch.empty(len(positions), dtype=torch.float)
    for query_idx, pos in enumerate(positions):
        num_gt = min(len(pos), k)
...
def test_on_exact_case() -> None:
    ...
    metrics_expected = defaultdict(dict)
...
def test_on_synthetic_cases(max_num_labels: int, max_num_samples_per_label: int, num_attempts: int) -> None:
    top_k = (1, 2, 3, 4, 5)

    for _ in range(num_attempts):
        case = generate_retrieval_case(max_labels=max_num_labels,
        ...
        metrics_expected["cmc"] = {k: _naive_cmc(positions, k) for k in top_k}
        metrics_expected["precision"] = {k: _naive_precision(positions, k) for k in top_k}
        metrics_expected["map"] = {k: _naive_map(positions, k) for k in top_k}

        compare_metrics(positions, labels, is_query, is_gallery, metrics_expected, top_k, reduce_mean=True)


def compare_metrics(positions: TPositions,
                    labels: torch.Tensor,
                    is_query: torch.Tensor,
                    is_gallery: torch.Tensor,
                    metrics_expected: TMetricsDict,
                    top_k: Tuple[int, ...],
                    reduce_mean: bool) -> None:
    ...
    metrics_calculated = calc_retrieval_metrics(distances=distances,
                                                mask_gt=mask_gt,
